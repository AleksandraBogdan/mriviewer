...
export default class VolumeFilter3d {
  ...
  */
  ...
  /* erasePixelsFloodFillRecursive(x_, y_, z_, startflag, mouseup, undoFlag, recursion, intensityTarget) {
    let targetX;
    let targetY;
    let targetZ;
    if (!undoFlag) {
      if (recursion) {
        ...
        intensityTarget = this.getIntensity(targetX, targetY, targetZ, undoFlag);
      }
    ...
    //console.log(`Target intensity: ${intensityTarget}`);
    //const stack = [];
    ...
    let xTmp = targetX;
    while (this.getIntensity(xTmp, targetY, targetZ, undoFlag) >= intensityTarget) {
      xTmp--;
    }
    ...
    for (xTmp = leftBound; xTmp <= rightBound; xTmp++) {
      this.changeIntensity(xTmp, targetY, targetZ, undoFlag);
      if (this.getIntensity(xTmp, targetY + 1, targetZ) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(xTmp, targetY + 1, targetZ, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(xTmp, targetY - 1, targetZ) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(xTmp, targetY - 1, targetZ, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(xTmp, targetY, targetZ + 1) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(xTmp, targetY, targetZ + 1, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(xTmp, targetY, targetZ - 1) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(xTmp, targetY, targetZ - 1, startflag, mouseup, undoFlag, true, intensityTarget);
      }
   ...
   /* let tmpX = targetX;
    while (this.getIntensity(tmpX, targetY, targetZ) >= intensityTarget) {
      this.changeIntensity(tmpX, targetY, targetZ, false);
      console.log(`${tmpX}, ${targetY}, ${targetZ}`);
      if (this.getIntensity(tmpX, targetY + 1, targetZ) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(tmpX, targetY + 1, targetZ, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(tmpX, targetY - 1, targetZ) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(tmpX, targetY - 1, targetZ, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(tmpX, targetY, targetZ + 1) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(tmpX, targetY, targetZ + 1, startflag, mouseup, undoFlag, true, intensityTarget);
      }
      if (this.getIntensity(tmpX, targetY, targetZ - 1) >= intensityTarget) {
        this.erasePixelsFloodFillRecursive(tmpX, targetY, targetZ - 1, startflag, mouseup, undoFlag, true, intensityTarget);
      }
  ...
  /* erasePixelsFloodFill(x_, y_, z_, startflag, mouseup, undoFlag) { //без времени
    let targetX;
    let targetY;
    let targetZ;
    if (!undoFlag) {
    ...
    while (stack.length !== 0) {
      ...
      console.log(`${point.tX}, ${point.tY}, ${point.tZ}`);
      let openUp = false;
      let openDown = false;
      let openFar = false;
      let openClose = false;
  ...
  /*erasePixelsFloodFill(x_, y_, z_, startflag, mouseup, undoFlag) { //засечение времени
    let targetX;
    let targetY;
    let targetZ;
    let countPoints = 100;// интервал который 10 или 100 точек
    //let unprocessedPoints = 0;
    let processedPoints = 0;
    const data = []; // список куда заношу данные
    let end = false;// флаг, чтоб выйти, если стек опустел
    while (!end) {
      const allTimes = [];// список куда заношу время
      for (let j = 0; j < 11; j++) {
        //unprocessedPoints = 0;
        processedPoints = 0;
        if (!undoFlag) {
        ...
        const time = performance.now();// засекаем начало
        ...
        const timeVoksel = performance.now();//засекаем конец
        allTimes.push(timeVoksel - time);
        this.resetBufferTextureCPU();
      }
      allTimes.sort();
      if (end) {
        data.push([processedPoints, allTimes[Math.ceil(allTimes.length / 2)]]);
        console.log(`processed points: ${processedPoints}`);
        //console.log(`unprocessed points: ${unprocessedPoints}`);
      } else {
        data.push([processedPoints, allTimes[Math.ceil(allTimes.length / 2)]]);
      }
      countPoints += 100; //увеличиваем количесвтво точек

    }
    if (!undoFlag) {
      const newData = data.map(el => `${[el[0], el[1]].join(';')}\r\n`);
  ...
  erasePixelsFloodFill(x_, y_, z_, startflag, mouseup, undoFlag) {
    ...
    while (!end) {
      ...
      for (let j = 0; j < 11; j++) {
        ...
        const intensityTarget = this.getIntensity(targetX, targetY, targetZ, undoFlag);
        console.log(`Target: ${intensityTarget}`);
        ...
        for (let i = 0; i < countInterval; i++) {
          ...
          unprocessedPoints++;
          const intensityPoint = this.getIntensity(point.tX, point.tY, point.tZ, undoFlag);

          if (intensityTarget <= intensityPoint) {
            processedPoints++;
            this.changeIntensity(point.tX, point.tY, point.tZ, undoFlag);
            ...
            idx = (point.tX + 1) + (point.tY + 1) * this.xDim + point.tZ * this.xDim * this.yDim;
            if (visited[idx] === 0) {
              queue.push({ 'tX': (point.tX + 1), 'tY': (point.tY + 1), 'tZ': point.tZ });
              visited[idx] = 1;
            }
            idx = (point.tX - 1) + (point.tY - 1) * this.xDim + point.tZ * this.xDim * this.yDim;
            if (visited[idx] === 0) {
            ...
            idx = (point.tX + 1) + point.tY * this.xDim + (point.tZ + 1) * this.xDim * this.yDim;
            if (visited[idx] === 0) {
              queue.push({ 'tX': (point.tX + 1), 'tY': point.tY, 'tZ': (point.tZ + 1) });
              visited[idx] = 1;
            }
            idx = (point.tX - 1) + point.tY * this.xDim + (point.tZ - 1) * this.xDim * this.yDim;
            if (visited[idx] === 0) {
            ...
            idx = point.tX + (point.tY + 1) * this.xDim + (point.tZ + 1) * this.xDim * this.yDim;
            if (visited[idx] === 0) {
              queue.push({ 'tX': point.tX, 'tY': (point.tY + 1), 'tZ': (point.tZ + 1) });
              visited[idx] = 1;
            }
            idx = point.tX + (point.tY - 1) * this.xDim + (point.tZ - 1) * this.xDim * this.yDim;
            if (visited[idx] === 0) {
        ...
        timeVoksel = performance.now() - startTime;
        allTimes.push(timeVoksel);
        this.resetBufferTextureCPU();
      }
      allTimes.sort();
      data.push([processedPoints, allTimes[Math.ceil((allTimes.length / 2))]]);
